...
import (
	...
	"data-platform-api-production-order-releases-rmq-kube/config"
...
func (c *DPFMAPICaller) headerRelease(
	...
	header.IsReleaseled = input.ProductionOrder.IsReleaseled
	...
	for i := range *items {
		(*items)[i].IsReleaseled = input.ProductionOrder.IsReleaseled
